...
# Reuse existing nodebuilder-beszel container, to speed up builds
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-beszel; then
    echo "Pulling NodeJS runtime..."
    buildah from --name nodebuilder-beszel -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi
...
# --label="org.nethserver.images=docker.io/mariadb:10.11.19 docker.io/roundcube/roundcubemail:1.6.12-apache"
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=docker.io/henrygd/beszel:0.18.3" \
    "${container}"
# Commit the image
